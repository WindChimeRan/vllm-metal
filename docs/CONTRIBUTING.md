...
editable Python sources and prebuilt native binaries without a compiler.
The installer checks that the checkout's C++ and Metal sources match the
release wheel. Use `--build` for native changes.
...
Locally built artifacts are rejected when their sources change without rebuilding.
