...
"""Editable installs reuse release binaries for Python-only changes."""
...
def test_reused_artifacts_replace_old_build_and_stamps(checkout):
    build.output_path().write_bytes(b"old build")
    build._stamp_path(build.output_path()).write_text("old stamp")
...
def test_python_edits_do_not_reject_prebuilt_artifacts(checkout):
    with release_wheel(checkout) as wheel:
        for name in ("build.py", "__init__.py"):
            path = checkout / name
