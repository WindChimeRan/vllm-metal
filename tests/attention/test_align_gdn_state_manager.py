...
def _make_cache(*, num_layers: int = 2, num_blocks: int = 8) -> GDNPagedStateCache:
...
class TestAlignGDNStateManager:
    ...
    def test_restore_on_hit_reads_checkpointed_block(self) -> None:
        cache = _make_cache()
        manager = AlignGDNStateManager(cache, BLOCK)
        _fill_slab(cache, 0, 1, 9.0)  # checkpoint left by an earlier request

        # Admitted with num_computed=4 (hit), scheduling 3 more tokens into
        # block idx 1; table row: [hit block 1, fresh block 5].
        ctx = self._populate(manager, ["req-B"], [[[1, 5]]], [(4, 3)])

        assert ctx.gdn_group_slot_mappings == ([5],)
        conv_dst, rec_dst = _slab(cache, 0, 5)
        np.testing.assert_array_equal(conv_dst, 9.0)  # restored from checkpoint
        conv_src, rec_src = _slab(cache, 0, 1)
        np.testing.assert_array_equal(conv_src, 9.0)  # checkpoint untouched
    ...
    def test_table_shorter_than_needed_raises(self) -> None:
        cache = _make_cache()
        manager = AlignGDNStateManager(cache, BLOCK)

        with pytest.raises(RuntimeError, match="needs state block index"):
            self._populate(manager, ["req-A"], [[[2]]], [(4, 1)])

    def test_release_is_a_noop_for_scheduler_owned_blocks(self) -> None:
        cache = _make_cache()
        manager = AlignGDNStateManager(cache, BLOCK)
        _fill_slab(cache, 0, 2, 5.0)
