...
def initialize_hybrid_runtime(runtime, num_blocks):
    ...
    attention = FullAttentionSpec(
        block_size=runtime._block_size,
        num_kv_heads=runtime._num_kv_heads,
        head_size=runtime._head_dim,
        dtype=MLX_TO_TORCH_DTYPE[runtime._dtype],
    )
    state = plan.state_cache_spec(
        mamba_block_size=runtime._block_size
        if runtime._mamba_cache_mode == "align"
        else 4096,
        page_size_padded=None,
        mamba_cache_mode=runtime._mamba_cache_mode,
    )
    ...
    config = SimpleNamespace(
        ...
        cache_config=make_cache_config(
            block_size=runtime._block_size, num_gpu_blocks_override=num_blocks
        ),
