...
class PagedAttentionRuntimeBase:
    ...
    def state_scheduler_group_indices(self) -> tuple[int, ...]:
        ...
        Empty for runtimes whose layers are all block-table addressed; the
        hybrid runtime returns its adopted mamba cache groups so the runner
        forwards those groups' block ids alongside the KV block tables.
        """
        return ()
