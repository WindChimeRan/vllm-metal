...
class GDNPagedStateCache:
    ...
    def layer_group_ordinal(self, cache_idx: int) -> int:
        ...
        ordinals = getattr(self, "_layer_group_ordinals", None)
        if ordinals is None:
            return 0
        return ordinals[cache_idx]

    def layers_for_group_ordinal(self, ordinal: int) -> list[int]:
        """Return the cache indices of layers in one mamba cache group."""
        ordinals = getattr(self, "_layer_group_ordinals", None)
        if ordinals is None:
            return list(range(self.num_layers)) if ordinal == 0 else []
        return [idx for idx, o in enumerate(ordinals) if o == ordinal]
