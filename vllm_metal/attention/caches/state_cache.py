...
class PagedStateCache:
    ...
    def __init__(self, states, group_ordinals=None):
        ...
        self.max_seqs = self.allocated_seqs = self.conv_states[0].shape[0]
        self.conv_kernel_dim = self.conv_states[0].shape[1] + 1
        self.conv_dim = self.conv_states[0].shape[2]
        self.dtype = self.conv_states[0].dtype
        if self.recurrent_states:
            recurrent = self.recurrent_states[0]
            self.num_v_heads, self.value_head_dim, self.key_head_dim = recurrent.shape[
                1:
            ]
            self.recurrent_dtype = recurrent.dtype
