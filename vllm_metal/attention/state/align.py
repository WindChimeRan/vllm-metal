...
state pool is therefore indexed directly by scheduler block id (one slab per
pool block, preallocated), and this manager owns the two per-step motions
...
class AlignGDNStateManager:
    ...
    def populate_step_context(
        ...
        *,
        ...
        ``(num_computed, num_scheduled)`` for this step.  The argument is
        optional only so both state managers share one signature; align
        mode cannot plan without it.
        """
        if step_positions is None:
            raise RuntimeError(
                "align-mode GDN state requires per-request "
                "(num_computed, num_scheduled) step positions"
            )
        ...
        ctx.gdn_slot_mapping = group_mappings[0] if group_mappings else []
